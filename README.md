...
분석이 성공하면 결과 영역의 `PDF로 저장` 버튼이 활성화됩니다. 버튼을 누르면 새 서버나 외부 서비스로 보고서를 보내지 않고 현재 브라우저의 인쇄 기능으로 **보고서 탭만** A4 레이아웃에 맞춘 인쇄 창에 표시합니다. 실제 용지 크기와 배율은 브라우저 설정이 우선합니다. 입력 폼, LM Studio 주소, 탐지 결과 탭과 요약 탭은 PDF에 포함되지 않습니다.
