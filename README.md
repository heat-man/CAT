...
3. 첫 청크에서 가장 이른 root 의심 프로세스와 관측된 행위를 요약하고, 이후 청크에는 직전의 **제한된 누적 사건 상태**를 전달해 부모·자식 실행, 지속성, DNS, 통신/C2 후보를 이어 봅니다.
...
각 중간 응답은 기본 2,048 token, 누적 상태는 8,192자, 최종 계층형 문맥은 20,480자로 제한됩니다. 최종 요청에도 `CAT_LM_MAX_INPUT_CHARS=49152`, 기본 출력에는 `CAT_LM_MAX_TOKENS=8192`가 적용됩니다. 이는 tokenizer 의존성 없이 1문자≈1token까지 보수적으로 보고 지시문을 포함한 기본 요청을 64k context 안에 두기 위한 조합이며, 128k 모델에서는 검증 후 명시적으로 늘릴 수 있습니다. 여러 청크의 원문은 최종 요청에 다시 한꺼번에 들어가지 않습니다. 청크 하나가 실패해도 경고를 기록하고 기존 CAT 대표 근거로 최종 보고서 생성을 계속 시도합니다. 청크 호출은 순차 LM 요청이며 `CAT_LM_TIMEOUT_SECONDS`가 각 청크와 최종 논리 호출에 각각 적용되므로, 전체 분석 시간은 단일 요청보다 길 수 있습니다.
...
`analysis.intrusion_chain`은 CAT가 로컬에서 결정적으로 만든 조사 보조 결과입니다. 의심 규칙과 로컬 명령 신호가 가장 강하게 연결된 프로세스 묶음을 고른 뒤, 그 안에서 확인되는 가장 이른 의심 프로세스를 `origin_process` 후보로 제시합니다. 이어지는 `processes`와 시간순 `steps`에는 가능한 범위에서 부모·자식 생성, 의심 행위, DNS와 네트워크 연결을 묶어 표시합니다.
...
- 일반 `최대 레코드` 이후의 Sysmon 1·3·5·22 및 Security 4688·4689·5156도 별도 endpoint spool이 완전하면 체인 후보와 후속 DNS/통신에 참여할 수 있습니다.
