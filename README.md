...
3. 기본으로 선택된 `경계 증거가 있으면 업로드한 파일 안에서 분석 시간대를 자동 확장`을 확인합니다. 이 옵션은 시간대(timezone)를 바꾸는 기능이 아니라 **분석 시간 범위**를 한 번만 넓혀 바로 앞·뒤 근거를 보충하는 기능입니다.
4. 필요한 경우 `최대 레코드`를 조정합니다. 기본값은 20,000개입니다.
5. `에이전트`에서 `LM Studio Qwen` 또는 `규칙 기반 보고서`를 선택합니다.
6. LM Studio를 선택했다면 URL과 `/v1/models`가 반환한 정확한 모델 ID를 확인합니다.
7. `분석 실행`을 누른 뒤 `보고서`, `탐지 결과`, `요약` 탭과 화면 상단의 경고를 확인합니다. `최초 침해 프로세스 후보`와 `침해 진행 체인`은 확정 판정이 아니라 원본 이벤트로 검증할 조사 출발점입니다.
8. 보고서를 보관하려면 `PDF로 저장`을 누르고 브라우저 인쇄 창에서 PDF 저장 대상을 선택합니다. 이전 결과는 `분석 이력`에서 다시 열 수 있습니다.
...
입력 폼, LM Studio URL, 탭 전환 버튼, 분석 이력 대화상자 같은 조작 UI는 PDF에 포함되지 않습니다. 화면의 `탐지 결과`와 `요약` 탭 자체는 숨기지만 그 핵심 분석 내용은 위 인쇄 전용 부록으로 다시 구성됩니다.
...
| `CAT_AUTO_EXPAND_TIME_RANGE` | `true` | 경계의 침해 징후를 따라 업로드 파일 안에서 한 번 범위 확장 허용 |
| `CAT_AUTO_EXPAND_EDGE_SECONDS` | `900` | 시작·종료 경계에서 확장 신호로 볼 거리, 60~3600초 |
| `CAT_AUTO_EXPAND_WINDOW_SECONDS` | `3600` | 한 번에 추가할 앞·뒤 분석 창, 300~86400초 |
...
1. `suspicious_events`, finding evidence, 침해 진행 체인, timeline과 대표 네트워크 그룹을 중복 제거하고 시간순으로 정렬합니다.
...
4. 최대 청크 수를 넘으면 전체 시간대를 대표하도록 각 구간의 경계와 위험도가 높은 근거를 우선 선택합니다. 일부 근거가 제외됐다는 사실은 metadata와 화면의 제한 안내에 남깁니다.
...
웹의 자동 확장 옵션과 `CAT_AUTO_EXPAND_TIME_RANGE=true`는 선택한 범위의 시작 또는 종료 경계 가까이에 실제 침해 후보 신호가 있고, 파서가 같은 업로드 파일에서 그 경계 밖 timestamp 이벤트를 확인했을 때만 동작합니다. 기본 경계 거리는 15분이고, 해당 방향으로 최대 1시간을 추가하되 업로드한 로그의 실제 최초·최종 시각을 넘지 않습니다. 확장은 **한 번만** 다시 파싱·분석합니다.

- 시작 경계 확장은 선행 부모·최초 실행 근거를, 종료 경계 확장은 자식 실행·DNS·후속 통신 근거를 보충하는 데 사용합니다.
- 외부 로그를 내려받거나 다른 장비에 추가 로그를 요청하지 않으며, 업로드한 파일 전체로 무조건 범위를 바꾸지 않습니다.
- 확장 파싱이 실패하면 최초 선택 범위의 분석 결과를 유지하고 오류를 metadata에 기록합니다.
- 결과의 `analysis.adaptive_time_range`와 요약/PDF에서 요청 범위, 실제 적용 범위와 확장 근거를 확인할 수 있습니다.
- 이는 `Asia/Seoul` 같은 로그 해석 **시간대(timezone)** 를 바꾸지 않습니다. 대형 파일을 다시 파싱하는 비용이 부담되면 웹 체크박스를 끄거나 `CAT_AUTO_EXPAND_TIME_RANGE=false`로 설정하세요.
...
| 선택한 시간보다 분석 범위가 넓어짐 | 자동 확장 사유와 요청/실제 범위를 요약에서 확인합니다. 업로드 파일 안의 경계 근거를 한 번 보충한 결과이며, 원치 않으면 웹 체크박스 또는 `CAT_AUTO_EXPAND_TIME_RANGE=false`로 끕니다. |
